...
namespace todo.web.Controllers
{
    public class ListController : BaseController
    {
        ...
        [HttpDelete("/{culture:regex(en)}/lists/{listId}/items/{itemId}/delete")]
        [HttpDelete("/{culture:regex(fr)}/listes/{listId}/elements/{itemId}/supprimer")]
        public async Task<IActionResult> DeleteItem(int id, int itemId)
        {
            ...
            if (!result.IsSuccessStatusCode)
                throw new ResultException($"Unable to delete item {itemId} of list {id} for {UserName}", result);

            SetSuccessMessage();
            return RedirectToAction(nameof(GetLists), new { itemId });
        }
